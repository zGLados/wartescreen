...
async function calculateStats(playerId) {
    ...
    for (let i = 0; i < maxMatches; i++) {
        ...
        // Debug: Log match structure for first match
        if (i === 0) {
        ...
        if (match.results && match.teams && match.teams.faction1 && match.teams.faction2) {
            ...
            if (match.results.winner === 'faction1' || match.results.winner === 'faction2') {
                ...
                if (i < 3) {
                    console.log(`Match ${i}: playerFaction=${playerFaction}, winner=${match.results.winner}, counted=${playerFaction === match.results.winner}`);
                }
